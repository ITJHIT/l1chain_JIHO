...
func cmdNode(args []string) error {
	...
	listenHost := fs.String("listen-host", "127.0.0.1", "libp2p listen host, TCP and QUIC (use 0.0.0.0 to be reachable from sibling Docker containers; loopback is not)")
	listen := fs.Int("listen", 0, "libp2p listen port, TCP and QUIC on the same port number (0 = ephemeral)")
